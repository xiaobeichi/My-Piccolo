...
namespace Piccolo
{
    ...
    void MotorComponent::tickPlayerMotor(float delta_time)
    {
        ...
        if (animation_component != nullptr)
        {
            ...
            animation_component->updateSignal("jumping",
                                              m_jump_state == JumpState::rising || m_jump_state == JumpState::falling);
        }
    }
    ...
    void MotorComponent::calculatedDesiredVerticalMoveSpeed(unsigned int command, float delta_time)
    {
        ...
        if (m_jump_state == JumpState::idle && m_controller->isTouchGround() == true)
        {
            ...
            else
            {
                m_vertical_move_speed = 0.f;
                 ...
                 m_controller->isTouchGround() == false)
        {
            m_vertical_move_speed -= gravity * delta_time;
            if (m_vertical_move_speed <= 0.f && m_controller->isFalling() == true)
            {
