...
namespace Piccolo
{
    ...
    void AnimationComponent::blend1D(float desired_ratio, BlendSpace1D* blend_state)
    {
        ...
        const auto& key_it    = m_signal.find(blend_state->m_key); // speed
        double      key_value = 0;
    ...
    void AnimationComponent::tick(float delta_time)
    {
        ...
        for (auto clip : m_animation_res.m_clips)
        {
            if (clip->m_name == name)
            {
                if (clip.getTypeName() == "BlendSpace1D") // important
                {
    ...
    void AnimationComponent::blend(float desired_ratio, BlendState* blend_state)
    {
        // int max_weight = 0;
        // int max_weight_i = 0;
        // for (int i = 0; i < blend_state->m_clip_count; i++){
        //     if(blend_state->m_blend_weight[i]>max_weight){
        //         max_weight = blend_state->m_blend_weight[i];
        //         max_weight_i = i;
        //     }
        // }
        // 应该设置一样的时间长度，那应该设置哪个时间长度呢，设置weight最大的可能把比较好
        ...
        // for (int i = 0; i < blend_state->m_clip_count; i++)
        // {
        //     blend_state->m_blend_ratio[i] =
        //         desired_ratio * blend_state->getLength() / blend_state->m_blend_clip_file_length[i];
        // }
