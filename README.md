# My Piccolo
- Modified from games104's piccolo engine
- Add **colorgrading** post-processing code to achieve the screen color conversion
- Add the **FSM state machine method for character animation**, able to change the motor movement state through the input and then switch the movement state through the state, the state includes: standing, walking, running mixed state, three jumping state
- Add a **blend-state animation mixing method** that interpolates between several animations, e.g., idle, walk, run
- Add **an automatic way to go up and down stairs** to the **character's controllor**, automatically going up and down stairs within a certain height, going up beyond that height will get stuck, and going down will become a jump
- Add a way for **character controllers to slide against a wall** to be able to move along a component parallel to the wall in the direction of movement without getting stuck while attached to the wall.
- Modify the lua component to **add the Attack component** as another component of the character, the component includes two fields: blood and attack power. Set the character's attributes via json reflection, or you can modify them in the editor. In addition, by modifying the input component to read the user input and pass it to the Attack component, you can realize that the **blood field will be increased and decreased when you press E,R respectively**.

# Piccolo Engine (formerly Piccolo Engine)
